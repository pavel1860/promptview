...
from promptview.model2.postgres.query_builders2 import Condition, TableField, TableName, Where
...
from promptview.model2.postgres.query_set2 import SelectQuerySet
...
@pytest.mark.asyncio
async def test_query_builders_with_filter():
    table_name = TableName("test")

    condition = Condition(TableField(table_name, "name"), "=", 1)
    cond_str = condition.render()
    # print(cond_str)
    assert cond_str == "name = 1"
    table_name.set_alias("t")
    cond_str = condition.render()
    # print(cond_str)
    assert cond_str == "t.name = 1"
    table_name.set_alias("tt")
    cond_str = condition.render()
    # print(cond_str)
    assert cond_str == "tt.name = 1"

    table_name = TableName("test")
    where = Where([Condition(TableField(table_name, "name"), "=", 1)], table_name)
    assert where.render() == "WHERE name = 1\n"
    table_name.set_alias("t")
    where = Where([Condition(TableField(table_name, "name"), "=", 1)], table_name)
    assert where.render() == "WHERE t.name = 1\n"
    table_name2 = TableName("test2")
    where = Where([Condition(TableField(table_name, "name"), "=", TableField(table_name2, "name"))], table_name)
    assert where.render() == "WHERE t.name = name\n"
    table_name2.set_alias("t2")
    where = Where([Condition(TableField(table_name, "name"), "=", TableField(table_name2, "name"))], table_name)
    assert where.render() == "WHERE t.name = t2.name\n"